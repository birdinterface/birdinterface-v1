...
import { Calendar, Check, ChevronDown, ChevronUp, Repeat } from "lucide-react"
import React, {
  KeyboardEvent,
  TouchEvent,
  useCallback,
...
function parseRecurrencePattern(input: string): ParsedPattern {
  ...
  const pluralDaysMatch = processed.match(
    new RegExp(`^((?:${dayPattern}s)(?:,\\s*(?:${dayPattern}s))*)$`)
  )
  if (pluralDaysMatch) {
    ...
    if (days.length > 0) {
      ...
      const formattedDays = sortedDays.map((d) =>
        Object.keys(dayNames).find((k) => dayNames[k] === d)
      )
      const suggestion =
  ...
  const singleDayMatch = processed.match(
    new RegExp(`^every\\s+(${dayPattern})$`)
  )
  if (singleDayMatch) {
  ...
  const andPattern = processed.match(
    new RegExp(`^every\\s+(${dayPattern})\\s+and\\s+(${dayPattern})$`)
  )
  if (andPattern) {
    ...
    const daysList = days
      .map((d) => Object.keys(dayNames).find((k) => dayNames[k] === d))
      .join(" and ")
    return {
      type: "days_of_week",
      daysOfWeek: days,
  ...
  const multipleDaysMatch = processed.match(
    new RegExp(`^every\\s+((?:${dayPattern})(?:,\\s*(?:${dayPattern}))*)$`)
  )
  ...
  const commaAndPattern = processed.match(
    new RegExp(
      `^every\\s+((?:${dayPattern})(?:,\\s*(?:${dayPattern}))*)\\s+and\\s+(${dayPattern})$`
    )
  )
  if (commaAndPattern) {
    const daysStr = commaAndPattern[1]
    const lastDay = commaAndPattern[2]
    const days = [...daysStr.split(",").map((d) => d.trim()), lastDay]
      .map((d) => dayNames[d])
      .filter((d) => d !== undefined)
    if (days.length > 0) {
      const sortedDays = days.sort()
      const daysList = sortedDays.map((d) =>
        Object.keys(dayNames).find((k) => dayNames[k] === d)
      )
      const formattedList =
        daysList.length > 2
          ? daysList.slice(0, -1).join(", ") +
            " and " +
  ...
  const directAndPattern = processed.match(
    new RegExp(`^(${dayPattern}s?)\\s+and\\s+(${dayPattern}s?)$`)
  )
  if (directAndPattern) {
  ...
  const shortAbbrevsMatch = processed.match(
    /^([a-z]{2,3})(?:,\s*([a-z]{2,3}))+$/
  )
  if (shortAbbrevsMatch) {
    ...
    const days = shortAbbrevs
      .map((abbrev) => shortToFull[abbrev])
      .filter((day) => day && dayNames[day] !== undefined)
      .map((day) => dayNames[day])

    if (days.length > 0) {
      const sortedDays = days.sort()
      const formattedDays = sortedDays.map(
        (d) => Object.keys(dayNames).find((k) => dayNames[k] === d) + "s"
      )
      const suggestion =
        formattedDays.length > 2
          ? formattedDays.slice(0, -1).join(", ") +
...
function getSuggestions(input: string): string[] {
  ...
  if (
    "weekdays".startsWith(normalized) &&
    normalized !== "weeke" &&
    normalized !== "weekl"
  ) {
    return ["weekdays"]
    ...
    for (const num of allNumbers) {
      if (afterEvery.startsWith(num)) {
        ...
        if (rest === "" || rest === " ") {
          ...
          const originalAfterEvery = input
            .substring(input.toLowerCase().indexOf("every") + 5)
            .trim()
  ...
  if (numberMatch) {
    ...
    if (num >= 2 && num <= 365) {
      ...
      if (afterNumber.length > 0) {
        ...
        const unit = afterNumber.startsWith(" ")
          ? afterNumber.substring(1)
          : afterNumber
...
export function RecurringTaskList({
  ...
  const [internalTasks, setInternalTasks] = useState<RecurringTask[]>(
    externalTasks || []
  )
      ...
      : [
          {
            ...
            dueDate: emptyTaskData.dueDate,
            completed: false,
            status: "",
            userId: "",
            recurrencePattern: emptyTaskData.recurrencePattern,
          },
        ]
  ...
  useEffect(() => {
    if (maintainFocusRef.current && focusedTaskPositionRef.current >= 0) {
      ...
      if (
        sortedTasks[targetPosition] &&
        sortedTasks[targetPosition].id !== "empty"
      ) {
  ...
  const updateTask = (id: string, updates: Partial<RecurringTask>) => {
    if (id === "empty") {
      setEmptyTaskData((prev) => ({
        ...
        description:
          typeof updates.description === "string"
            ...
            : prev.description,
        recurrencePattern:
          typeof updates.recurrencePattern === "string"
            ? updates.recurrencePattern
            : prev.recurrencePattern,
        dueDate:
          typeof updates.dueDate === "string" ? updates.dueDate : prev.dueDate,
      }))
      ...
      if (
        ...
        const parsedPattern = parseRecurrencePattern(
          currentTask.recurrencePattern
        )
        if (parsedPattern.type !== "invalid") {
          const currentDueDate = parseTaskDate(currentTask.dueDate)
          if (currentDueDate) {
            const nextOccurrence = findNextOccurrenceAfterDate(
              currentDueDate,
              parsedPattern
            )
            if (nextOccurrence) {
    ...
    if (onDeleteTask) {
      ...
      requestAnimationFrame(() => {
        ...
        const taskInputs = document.querySelectorAll(
          ".recurring-task-container .task-input[data-task-id]"
        )
    ...
    <div className="w-full flex items-start justify-center recurring-task-container">
      <div className="w-full max-w-2xl border border-task-border rounded-t-md rounded-b-2xl mb-4 bg-normal-task">
        <div className="p-4">
          ...
          >
            <span className="text-xs text-foreground task-tab ml-2">
              Recurring Tasks
            </span>
            {isCollapsed ? (
          ...
          <div className="space-y-2 py-0 px-4 pb-4">
            {displayTasks.map((task) => (
              <div
                ...
                style={{
                  opacity:
                    taskToDelete === task.id ? 1 - swipeDistance / 100 : 1,
                }}
              >
                                ...
                                <CustomCalendar
                                  ...
                                  recurringDates={(() => {
                                    if (
                                      ...
                                      const parsed = parseRecurrencePattern(
                                        task.recurrencePattern
                                      )
...
function findNextDateForPattern(
  startDate: Date,
  pattern: ParsedPattern
): Date | null {
...
function findNextOccurrenceAfterDate(
  currentDate: Date,
  pattern: ParsedPattern
): Date | null {
  if (pattern.type === "invalid") return null
...
function RecurringSettings({
  ...
  const handleInputChange = (value: string) => {
    ...
    setShowSuggestion(
      newSuggestion.length > 0 && value.length > 0 && newSuggestion !== value
    )
  }
          ...
          onFocus={() => {
            ...
            setShowSuggestion(
              newSuggestion.length > 0 &&
                (task.recurrencePattern || "").length > 0 &&
                newSuggestion !== (task.recurrencePattern || "")
            )
          }}
          onBlur={() => {
