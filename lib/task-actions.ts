import { createTask, deleteTask, updateTask } from "@/lib/queries"
...
export function generateTaskSuggestions(tasks: Task[]): TaskSuggestion[] {
  ...
  const tasksWithoutDueDates = tasks.filter(
    (task) => !task.completed && !task.dueDate
  )

  if (tasksWithoutDueDates.length > 0) {
    const taskTitles = tasksWithoutDueDates
      .slice(0, 5)
      .map((t) => `"${t.title}"`)
      .join(", ")
    const moreText =
  ...
  const vagueDescriptionTasks = tasks.filter(
    (task) =>
      !task.completed && (!task.description || task.description.length < 10)
  )
...
export function formatTaskSuggestionsForAI(
