...
export const validateSearchParameters = (
  ...
  if (
    ...
    (params.max_search_results < 1 || params.max_search_results > 50)
  ) {
  ...
  if (params.sources) {
    params.sources.forEach((source, index) => {
      ...
      if (source.excluded_websites && source.allowed_websites) {
        errors.push(
          `Source ${index}: cannot use both excluded_websites and allowed_websites`
        )
      }

      if (source.type === "rss" && source.links && source.links.length > 1) {
        errors.push(
          `Source ${index}: RSS sources currently support only 1 link`
        )
      }
    })
  }
