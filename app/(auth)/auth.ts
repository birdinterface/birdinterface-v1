    ...
    async signIn({ user, account }) {
      if (account?.provider === "google") {
        try {
          ...
          if (dbUser.length === 0) {
            await createUser(user.email!, null, user.name || "")
            ...
            user.stripesubscriptionid =
              newUser.stripesubscriptionid || undefined
            ...
            user.stripesubscriptionid =
              existingUser.stripesubscriptionid || undefined
    ...
    async jwt({ token, user, trigger, session }) {
      ...
      if (trigger === "update" && session?.user) {
        ...
        token.stripesubscriptionid =
          session.user.stripesubscriptionid || undefined
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.stripecustomerid = token.stripecustomerid as
          | string
          | undefined
        session.user.stripesubscriptionid = token.stripesubscriptionid as
          | string
          | undefined
